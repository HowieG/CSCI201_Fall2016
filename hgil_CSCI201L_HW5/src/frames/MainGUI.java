...
import other_gui.ImageAnimator;
...
public class MainGUI extends JFrame
{
	...
	public ImageAnimator clockAnimation;
	public static JLabel timerLabel;
	public JLabel updatesLabel;

	public static Vector<JPanel> clockPanels;
	//	public static JPanel pointsPanel;
	...
	private JPanel createJeopardyPanel()
	{
		...
		JLabel jeopardyLabel = new JLabel();
		...
		timerLabel.setFont(AppearanceConstants.fontLarge);
	...
	private JPanel createProgressPanel()
	{
		...
		updatesLabel = new JLabel("Game Progress" + gameData.getCurrentTeam().getTeamName());
		...
		{
			ImageAnimator newClock = new ImageAnimator(143, 15, "Clock");
			clockPanels.add(newClock);
			...
			//			testPanel = clockAnimation;
			pointsPanel.add(clockPanels.get(i));

			if (team.getTeamIndex() == gameData.getCurrentTeam().getTeamIndex())
			{
				clockPanels.get(i).setVisible(true);
			}
	...
	public void updateClockPanels(int teamThatShowsClock)
	{
		for (int i = 0; i < gameData.getNumberOfTeams(); i++)
		{
			TeamGUIComponents team = gameData.getTeamDataList().get(i);
			clockPanels.get(i).setVisible(false);
			if (team.getTeamIndex() == teamThatShowsClock)
			{
				clockPanels.get(i).setVisible(true);
			}
		}
